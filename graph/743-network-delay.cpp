// LeetCode 743：网络延迟时间
// 题意：n 个节点编号为 1～n，times 中 [u, v, w] 表示有向边及非负传输耗时。
//       信号从 k 出发，求所有节点收到信号的最早时间；有节点不可达则返回 -1。
// 示例：times = [[2, 1, 1], [2, 3, 1], [3, 4, 1]]，n = 4，k = 2，返回 2。
// 思路：构建邻接表，用小顶堆实现 Dijkstra；弹出距离最小的状态，跳过过期
//       状态，并松弛相邻边。最终取各节点最短距离的最大值，不可达时返回 -1。
// 复杂度：时间 O((n + E) log(n + E))，额外空间 O(n + E)，E 为边数。
