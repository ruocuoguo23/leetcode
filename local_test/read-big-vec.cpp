// 本地练习：十进制大整数加法与大数组分页草稿
// 目标：当前可执行部分计算两个非负整数字符串之和，避免依赖内置整数的数值范围。
// 示例："99999998" + "1" 得到 "99999999"。
// 思路：将字符转成数字数组并反转，从低位逐位相加和处理进位，再处理较长数
//       剩余位及最终进位，最后反转结果字符串。时间、空间均为 O(m + n)。
// 实现状态：输入按非空数字串处理；文件内大数组分页与缓存部分仍是注释草稿。
