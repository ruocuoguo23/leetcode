// 本地练习：检查二维网格中的封闭区域（根据当前代码概括，未标注题目来源）
// 目标：在 0、1 网格中寻找被 1 围住的 0 区域，代码采用上下左右连通关系。
// 思路：遍历未访问的 0，递归搜索连通区域；对每个格子沿上下左右扫描，
//       检查四个方向是否都能遇到 1，用 has_curve 汇总判断。
// 实现限制：搜索发现失败后会提前返回，visited 可能只覆盖部分连通块；之后重新
//       搜索剩余部分可能误判，因此此实现仍需结合边界连通的反例验证。
